...
import { Subjects, QueueGroupName } from "../index";
...
export abstract class Listener<T extends Event> {
  ...
  listen() {
    if (!this.queueGroupName)
      throw new Error("QueueGroupName is not configured");
    this.client
      ...
      .on("message", (msg: CustomMessage) => {
        ...
        console.log(
          `Message received: ${this.subject} / ${this.queueGroupName} - parentId: ${msg.parentId}`
        );
    ...
    this.client
      .subscribe(
        `[REPLAY]-${this.subject}-${this.queueGroupName}`,
        this.queueGroupName,
        this.subscriptionOptions_replay()
      )
      .on("message", (msg: CustomMessage) => {
        const parsedBody = this.parseMessage(msg);
        msg.queueGroupName = this.queueGroupName;
        ...
        msg.subject = this.subject;
        console.log(
          `[REPLAY]Message received: ${this.subject} / ${this.queueGroupName} - parentId: ${msg.parentId}`
        );
        this.onMessage(parsedBody.data, msg.parentId, msg);
      });
  }
...
const fail = async function (err: Error, allowedFails: number = 3) {
  console.log(allowedFails);
  console.error(
    `${this.parentId} event failed, queueGroupName: ${this.queueGroupName}, errMsg: ${err.message}`
  );
